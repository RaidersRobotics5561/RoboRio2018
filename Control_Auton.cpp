...
T_AutonOpt DtrmnAutonOption(T_RobotSide L_AutonTargetSwitch,
		T_RobotSide L_AutonTargetScale, T_AutonStartPos L_AutonStartPos,
		T_AutonPreference L_AutonPreference, bool AutonCrossover) {
...
//Test to see if we want Left Path
	...
	if (L_AutonStartPos == E_AutonStartPosRight) {
		...
		SmartDashboard::PutBoolean("Crossover",AutonCrossover);
 ...
 *
 ******************************************************************************/
bool DtrmnActuatorComplete(double L_CntrlVal, double L_MeasuredVal,
		double L_Deadband, double *L_DebounceTimer,
	...
	if ((L_MeasuredVal >= L_DeadbandLow) && (L_MeasuredVal <= L_DeadbandHigh)) {
 ...
 *
 ******************************************************************************/
bool DtrmnActuatorCompleteSonic(double L_CntrlVal, double L_MeasuredVal,
		double L_Deadband, double *L_DebounceTimer,
		double L_DebounceThreshold) {
	bool L_ControlComplete = false;
...
double CntrlAutonDesiredSpeed(double L_K_TotalPlannedTravel,
		double L_MeasuredTravel, double L_DesiredSpeedPrev,
		double L_K_RampDownTravel, double L_K_MinSpeed, double L_K_MaxSpeed,
		double L_K_SpeedRamp, double L_DebounceTime, bool L_TargetMet) {
	...
	L_RampDownDistance = fabs(L_K_TotalPlannedTravel)
			- fabs(L_K_RampDownTravel);
	...
	if ((L_TargetMet == true) || (L_DebounceTime > 0.0)) {
...
double CntrlAutonUltraSonic(double L_K_TotalPlannedTravel,
		double L_MeasuredTravel, double L_K_MinSpeed, bool L_TargetMet) {
	double L_DesiredSpeed = 0.0;
	double L_Error = 0.0;
...
double CntrlAutonDesiredRotate(double L_K_TotalPlannedRotation,
		double L_MeasuredRotation, double L_DesiredRotationPrev,
		double L_K_ProportionalGx, double L_K_MinSpeed, double L_K_MaxSpeed,
		double L_DebounceTime, bool L_TargetMet) {
	double L_DesiredSpeed = 0.0;
	double L_Error = 0.0;
...
double CntrlAutonDesiredLift(double L_K_TotalPlannedTravel,
		double L_MeasuredTravel, double L_DesiredHeightPrev,
		double L_K_DesiredTravelRamp, bool L_TargetMet) {
	...
	if (L_TargetMet == true) {
		...
		L_DesiredHeight = L_DesiredHeightPrev
				- (L_K_DesiredTravelRamp * C_ExeTime);
 ...
 *
 ******************************************************************************/
bool CntrlAutonDrive(T_Actuator L_CntrlActuator, double L_AutonTarget) {
	bool L_ControlComplete = false;
	T_RobotSide L_RobotSide = E_RobotSideLeft;
	float L_RobotPID_Gx[E_RobotSideSz][E_PID_Sz];

	L_RobotPID_Gx[E_RobotSideLeft][E_PID_Proportional] =
			K_WheelSpeedPID_GainAuton[E_RobotSideLeft][E_PID_Proportional];
	L_RobotPID_Gx[E_RobotSideLeft][E_PID_Integral] =
			K_WheelSpeedPID_GainAuton[E_RobotSideLeft][E_PID_Integral];
	L_RobotPID_Gx[E_RobotSideLeft][E_PID_Derivative] =
			K_WheelSpeedPID_GainAuton[E_RobotSideLeft][E_PID_Derivative];
	L_RobotPID_Gx[E_RobotSideRight][E_PID_Proportional] =
			K_WheelSpeedPID_GainAuton[E_RobotSideRight][E_PID_Proportional];
	L_RobotPID_Gx[E_RobotSideRight][E_PID_Integral] =
			K_WheelSpeedPID_GainAuton[E_RobotSideRight][E_PID_Integral];
	L_RobotPID_Gx[E_RobotSideRight][E_PID_Derivative] =
			K_WheelSpeedPID_GainAuton[E_RobotSideRight][E_PID_Derivative];

	if (L_CntrlActuator == E_ActuatorDriveEncoder) {
		L_ControlComplete = DtrmnActuatorComplete(L_AutonTarget,
				V_DistanceTraveledAvg, K_AutonDriveDistanceDeadband,
				&V_AutonWheelDebounceTimer[E_RobotSideRight],
				K_AutonDebounceThreshold);

		V_WheelRPM_Desired[E_RobotSideRight] = CntrlAutonDesiredSpeed(
				L_AutonTarget, V_DistanceTraveledAvg,
				V_WheelRPM_Desired[E_RobotSideRight],
				K_AutonDriveDistanceToSlow, K_AutonDriveMinSpeed,
				K_AutonDriveMaxSpeed, K_AutonDriveSpeedRamp,
				V_AutonWheelDebounceTimer[E_RobotSideRight], L_ControlComplete);
		...
		for (L_RobotSide = E_RobotSideLeft; L_RobotSide < E_RobotSideSz;
			...
			V_WheelRPM_Desired[L_RobotSide] = CntrlAutonUltraSonic(
					L_AutonTarget, V_UltraSonicDistance[L_RobotSide],
					K_AutonDriveMinSpeedUltra, L_ControlComplete);

		}
	} else if (L_CntrlActuator == E_ActuatorRotate) {
		L_RobotPID_Gx[E_RobotSideLeft][E_PID_Proportional] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideLeft][E_PID_Proportional];
		L_RobotPID_Gx[E_RobotSideLeft][E_PID_Integral] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideLeft][E_PID_Integral];
		L_RobotPID_Gx[E_RobotSideLeft][E_PID_Derivative] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideLeft][E_PID_Derivative];
		L_RobotPID_Gx[E_RobotSideRight][E_PID_Proportional] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideRight][E_PID_Proportional];
		L_RobotPID_Gx[E_RobotSideRight][E_PID_Integral] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideRight][E_PID_Integral];
		L_RobotPID_Gx[E_RobotSideRight][E_PID_Derivative] =
				K_WheelSpeedPID_GainRotateAuton[E_RobotSideRight][E_PID_Derivative];

		L_ControlComplete = DtrmnActuatorComplete(L_AutonTarget,
				V_GyroAngleRelative, K_AutonRotateAngleDeadband,
				&V_AutonRotateDebounceTimer, K_AutonDebounceThreshold);

		V_WheelRPM_Desired[E_RobotSideLeft] = CntrlAutonDesiredRotate(
				L_AutonTarget, V_GyroAngleRelative,
				V_WheelRPM_Desired[E_RobotSideLeft], K_AutonRotateAnglePropGx,
				K_AutonRotateMinSpeed, K_AutonRotateMaxSpeed,
				V_AutonRotateDebounceTimer, L_ControlComplete);
	...
	for (L_RobotSide = E_RobotSideLeft; L_RobotSide < E_RobotSideSz;
			L_RobotSide = T_RobotSide(int(L_RobotSide) + 1)) {
		V_RobotMotorCmndPct[L_RobotSide] =
 ...
 *
 ******************************************************************************/
bool CntrlAutonLift(T_Actuator L_CntrlActuator, double L_AutonTarget) {
	bool L_ControlComplete = false;

	L_ControlComplete = DtrmnActuatorComplete(L_AutonTarget, V_IntakePosition,
			K_AutonIntakeDistanceDeadband, &V_AutonIntakeLiftDebounceTimer,
			K_AutonDebounceThreshold);

	V_IntakeLiftHeightDesired = CntrlAutonDesiredLift(L_AutonTarget,
			V_IntakePosition, V_IntakeLiftHeightDesired, K_AutonIntakeRamp,
			L_ControlComplete);
 ...
 *
 ******************************************************************************/
bool CntrlAutonOpenLoopTimer(T_Actuator L_CntrlActuator, double L_AutonTarget) {
	bool L_ControlComplete = false;
